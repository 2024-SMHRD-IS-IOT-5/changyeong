...
export default class Ex13 extends Component {
    ...
    때문에 이를 useEffect라는 리액트훅으로 대체하게 된다.
    */

    /* LifeCycle
    - 리액트는 컴포넌트 기반의 View 중심 라이브러리
    - 각각의 컴포넌트에는 제각각 수명주기(LifeCycle) 이라는 게 존재
    - 컴포넌트가 생겨나고, 변화하고, 없어지는 일련의 프로세스를
        프로그래머가 통제하는 것이 LifeCycle의 핵심

    1. constructor (클래스 생성자)
    - 리액트에서 컴포넌트를 만들 때 처으믕로 실행됩니다.
    - 초기 state, 변수, 함수 등이 이곳에서 선언된다.
    
    2. render (작업한 결과물을 return 해주는 렌더)
    - ⭐필수 메소드⭐ <- 얘만 유일하게 필수, 나머지는 편의상 쓰는거
    - render 메소드가 실행되면서 우리가 작성한 JSX 가 HTML로 변환되어
    우리 눈에 보여지게 됨
    ...
    속도가 빠르다고 느낌, 그렇기 때문에 일부 화면을 띄운 후 데이터, API를
    가져오는 것
